...
"$jmeter" -n -t ./upload-maven-csv.jmx -l ./artipie-upload.log -e -o ./artipie-upload-res -Jrepository.host="$host" -Jrepository.port="$port" \
  -Jrepository.path=ChGen/maventest -Jduration="$duration" -Jsrc.path=upload-maven-src -Jsrc.list=upload-maven.csv
mv -f artipie-upload.log artipie-upload-res
mv -f artipie-upload-res "maven_${host}_${port}_${duration}_$(date +%y-%m-%d_%H-%M-%S)"
